 ...
 ******************************************************************************/
package it.proximacentauri.index.ienergy2.core;
...
import it.proximacentauri.index.ienergy2.json.Measure;
import it.proximacentauri.index.ienergy2.json.MeasureDescriptor;
...
public class IEnergyAdapter implements BalanceAdapter {
	...
	static private String DATETIME_FORMAT = "yyyy-MM-dd'T'HH:mm:ssZ";
	...
	private List<BalanceSnapshot> loadBalanceSnaphost(BalanceItem item, Date start, Date end, Cadency cadency) {
		...
		try {
			...
			MeasureDescriptor descriptor = mapper.readValue(response.getEntity().getContent(), MeasureDescriptor.class);

			// logger.trace("{}", descriptor);

			for (Measure measure : descriptor.getMeasures()) {
				final BalanceSnapshot snapshots = new BalanceSnapshot();
				snapshots.setDatetime(measure.getTime());
				snapshots.setValue(measure.getValue());
	...
	private String buildUrl(BalanceItem item, Date start, Date end) throws UnsupportedEncodingException {
		...
		// append start time and end time
		// 2013-01-18T09:17:18%2B0100
		final SimpleDateFormat format = new SimpleDateFormat(DATETIME_FORMAT);
		url += "?start=" + format.format(start).replace("+", "%2B");
		url += "&end=" + format.format(end).replace("+", "%2B");

		return this.url + url;
	}
	//
	// private List<BalanceSnapshot> loadBalanceSnaphost(BalanceItem item, Date
	// start, Date end, Cadency cadency) {
	// logger.info("load balance item value {}", item);
	//
	...
	// if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
	// throw new
	// UnableToCreateBalanceException("Error on load source -> the status code is code "
	// + response.getStatusLine().getStatusCode());
	// }
	//
	// // start parsing data
	// MdxResult mdx = mapper.readValue(response.getEntity().getContent(),
	// MdxResult.class);
	// logger.info("{}", mdx);
	//
	...
	// * @throws UnsupportedEncodingException
	// */
	// private String buildUrl(BalanceItem item, Date start, Date end) throws
	...
	// mdxString = mdxString.replaceAll(":startYear:",
	// Integer.toString(calendar.get(Calendar.YEAR)));
	// mdxString = mdxString.replaceAll(":startMonth:",
	// Integer.toString(calendar.get(Calendar.MONTH) + 1));
	// mdxString = mdxString.replaceAll(":startDay:",
	// Integer.toString(calendar.get(Calendar.DAY_OF_MONTH)));
	// mdxString = mdxString.replaceAll(":startHour:",
	// Integer.toString(calendar.get(Calendar.HOUR_OF_DAY)));
	//
	// calendar.setTime(end);
	// mdxString = mdxString.replaceAll(":endYear:",
	// Integer.toString(calendar.get(Calendar.YEAR)));
	// mdxString = mdxString.replaceAll(":endMonth:",
	// Integer.toString(calendar.get(Calendar.MONTH) + 1));
	// mdxString = mdxString.replaceAll(":endDay:",
	// Integer.toString(calendar.get(Calendar.DAY_OF_MONTH)));
	// mdxString = mdxString.replaceAll(":endHour:",
	// Integer.toString(calendar.get(Calendar.HOUR_OF_DAY)));
	//
