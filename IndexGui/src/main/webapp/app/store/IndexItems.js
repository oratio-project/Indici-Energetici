...
Ext.define('Index.store.IndexItems', {
	extend : 'Ext.data.Store',
	model : 'Index.model.IndexItem',
	autoLoad : true,

	proxy : {
		type : 'rest',
		url : Index.util.Utilities.serverAddress+'indices',
		reader : {
			type : 'json'
		},
		listeners : {
	...
	setCategory : function (category){
		if(category==-1){
			this.getProxy().url = Index.util.Utilities.serverAddress + 'indices';
			console.log(this.getProxy().url);
		}else{
			this.getProxy().url = Index.util.Utilities.serverAddress + 'indices?category='+category;
			console.log(this.getProxy().url);
		}
	}
});
